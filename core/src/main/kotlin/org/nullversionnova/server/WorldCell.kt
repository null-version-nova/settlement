...
class WorldCell (private val location: IntVector3) {
    ...
    fun generate(registry: ServerRegistry) {
        ...
        for (i in 0 until CELL_SIZE) {
            for (j in 0 until CELL_SIZE) {
                ...
                addColumn(IntVector3(i,j,height), SOIL_DEPTH, Z, SolidTile(Identifier("settlement","sand")),registry)
            }
        }
    }
    fun unload() {
        loaded = false
        tileMap.clear()
        tickableTileMap.clear()
    }
    fun findTile(location: IntVector3) : TileStorage? {
        for (i in tileMap) {
            if (i.overlap(location)) {
                return i
            }
        }
        for (i in tickableTileMap) {
            if (i.overlap(location)) {
                return i
            }
        }
    ...
    private fun addPlane(location: IntVector3, height: Int, width: Int, axis: Axis, tile: GameObject, registry: ServerRegistry) {
...
//                tilemap.add(TileColumn(location.getNewWithSetAxis(-location.getAxis(axis),axis),height - location.getAxis(axis),axis,tile))
    ...
    private fun addUnit(location: IntVector3, tile: GameObject, registry: ServerRegistry) {
        if (location.x >= 0 && location.y >= 0 && location.z >= 0 && location.x < CELL_SIZE && location.y < CELL_SIZE && location.z < CELL_SIZE) {
            if (tile.getTile(registry) is TickableTile) {
                tickableTileMap.add(TileUnit(location, tile))
            } else {
                tileMap.add(TileUnit(location, tile))
            }
        }
    }
    private fun remove(item: TileStorage) {
        if (tileMap.contains(item)) { tileMap.remove(item) }
        if (tickableTileMap.contains(item)) { tickableTileMap.remove(item) }
    }
